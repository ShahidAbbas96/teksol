...
export default function CTASection({
  ...
  return (
    <section className={isDark ? "bg-brand-dark py-20 sm:py-24" : "bg-white py-20 sm:py-24"}>
      <Container>
        <div
          className={`flex flex-col items-center gap-8 rounded-3xl px-6 py-14 text-center sm:px-16 ${
            isDark
              ? "bg-gradient-to-br from-[#1e2a44] to-[#141b2c] border border-white/10"
              : "bg-brand-background border border-slate-200"
          }`}
        >
          <h2 className={`max-w-2xl text-3xl font-bold tracking-tight sm:text-4xl ${isDark ? "text-white" : "text-brand-dark"}`}>
            {heading}
          </h2>
          <p className={`max-w-xl text-lg leading-relaxed ${isDark ? "text-slate-300" : "text-slate-600"}`}>
            {description}
          </p>
